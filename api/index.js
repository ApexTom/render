...
function generateGuid() { return randomBytes(16).toString('hex'); }

function calcSignature(bytes) {
  let sig = 0;
  for (const b of bytes) sig = uint32(uint32(0x83 * sig) + (b & 0xFF)) & 0x7FFFFFFF;
  return sig;
}
...
function teaEncryptECB(block8, keyBuf) {
  ...
  let y = block8.readUInt32BE(0), z = block8.readUInt32BE(4), sum = 0;
  ...
  out.writeUInt32BE(y, 0); out.writeUInt32BE(z, 4);
  return out;
}

function teaDecryptECB(block8, keyBuf) {
  const k = [keyBuf.readUInt32BE(0), keyBuf.readUInt32BE(4), keyBuf.readUInt32BE(8), keyBuf.readUInt32BE(12)];
  let y = block8.readUInt32BE(0), z = block8.readUInt32BE(4);
...
function oiSymmetryEncrypt2(plainBuf, keyBuf) {
  ...
  let nPadlen = (nInBufLen + 1 + SALT_LEN + ZERO_LEN) % 8;
  ...
  function flushBlock() {
    ...
    iv_plain.set(src_buf); iv_crypt = out;
    outChunks.push(Buffer.from(out)); src_i = 0;
  }

  for (let p = 0; p < nPadlen; p++) { src_buf[src_i++] = randomBytes(1)[0]; if (src_i===8) flushBlock(); }
  for (let s = 0; s < SALT_LEN; s++) { src_buf[src_i++] = randomBytes(1)[0]; if (src_i===8) flushBlock(); }
  for (let bi = 0; bi < nInBufLen; bi++) { src_buf[src_i++] = plainBuf[bi]; if (src_i===8) flushBlock(); }
  for (let z = 0; z < ZERO_LEN; z++) { src_buf[src_i++] = 0; if (src_i===8) flushBlock(); }
  if (src_i > 0) { for (let j = src_i; j < 8; j++) src_buf[j] = 0; flushBlock(); }

  return Buffer.concat(outChunks);
}

// -------------------- ck_guard_time --------------------
function guardLastFive(s) { s = String(s); return s.length >= 5 ? s.slice(-5) : ''; }
...
function generateCkGuardTime(timestamp, guid) {
  ...
  const tsBytes = Buffer.alloc(4); tsBytes.writeUInt32BE(timestamp >>> 0, 0);
  ...
  tsBytes.copy(body, 0);
  let pos = 4;
  for (const pb of partBufs) { body.writeUInt16BE(pb.length, pos); pos += 2; pb.copy(body, pos); pos += pb.length; }
  const plain = Buffer.alloc(2 + bodyLen);
  plain.writeUInt16BE(bodyLen, 0); body.copy(plain, 2);
  ...
  encrypted.copy(full, 0); full.writeUInt32BE(checksum >>> 0, encrypted.length);
...
function strField(s) {
  ...
  r.writeUInt16BE(sb.length, 0); sb.copy(r, 2);
  return r;
}

function buildPacket(params) {
  const plat = Buffer.alloc(4); plat.writeUInt32BE(params.Platform, 0);
  const sigPH = Buffer.alloc(4);
  const ts = Buffer.alloc(4); ts.writeUInt32BE(params.Timestamp, 0);
  const one = Buffer.alloc(4); one.writeUInt32BE(1, 0);

  const body = Buffer.concat([
    hexToBytes('0000004200000004000004d2'),
    plat, sigPH, ts,
    strField(params.Sdtfrom), strField(params.randFlag), strField(params.appVer),
    strField(params.vid), strField(params.guid),
    one, Buffer.from(one), // part1, isDlna
    strField('2622783A'), strField('nil'), strField(params.uuid4), strField('nil'),
    strField('v0.1.000'), strField('com.cctv.yangshipin.app.iphone'),
    strField('4330403'), strField('ex_json_bus'), strField('ex_json_vs'),
    strField(params.ck_guard_time),
  ]);

  const lenHdr = Buffer.alloc(2); lenHdr.writeUInt16BE(body.length, 0);
  const buffer = Buffer.concat([lenHdr, body]);
  buffer.writeUInt32BE(calcSignature(buffer) >>> 0, 18);
  return buffer;
}
...
function encryptDataToCKey(dataBuf) {
  ...
  const full = Buffer.alloc(encrypted.length + 4);
  encrypted.copy(full, 0); full.writeUInt32BE(checksum >>> 0, encrypted.length);
  return '--01' + customEncode(xorArray(full));
}
...
function generateCKey(cnlid, timestamp, guid) {
  ...
  const params = {
    ...
    vid: cnlid, guid, appVer: 'V8.22.1035.3031',
    randFlag: '_zj1A5Gh6QYcxWjIUGos2w==',
    uuid4: '57eab0c4-2c58-44c6-8ae9-dd2757525dc5',
    ck_guard_time,
  };
  return { ckey: encryptDataToCKey(buildPacket(params)), params };
}
...
async function getPlayUrl(cnlid, livepid, defn) {
  ...
  const qp = new URLSearchParams({
    atime:'120', livepid, cnlid, appVer:'V8.22.1035.3031', app_version:'300090',
    caplv:'1', cmd:'2', defn, device:'iPhone', encryptVer:'4.2',
    getpreviewinfo:'0', hevclv:'33', lang:'zh-Hans_JP', livequeue:'0',
    logintype:'1', nettype:'1', newnettype:'1', newplatform:'4330403', platform:'4330403',
    sdtfrom:'v3021', spacode:'23', spaudio:'1', spdemuxer:'6', spdrm:'2',
    spdynamicrange:'7', spflv:'1', spflvaudio:'1', sphdrfps:'60', sphttps:'0',
    ...
    cKey: ckey, guid, fntick: String(params.Timestamp),
    flowid: generateFlowid(), playbacktime:'0',
  });
...
function generateM3u(baseUrl) {
  const group = id => (id.startsWith('cctv')||id.startsWith('cgtn')) ? '央视' : (id==='cetv1'||id==='gxpd') ? '教育' : '卫视';
  let m3u = '#EXTM3U\n';
...
export default async function handler(req, res) {
  ...
  try {
    if (path === '/m3u' || path === '/playlist.m3u') {
      ...
      return res.end(generateM3u(baseUrl));
    }

    if (path === '/live') {
      const id = url.searchParams.get('id') || 'cctv1';
      const ch = CHANNELS[id];
      if (!ch) return res.status(404).end('未知频道');
      const [cnlid, livepid, defn] = ch;
      const playUrl = await getPlayUrl(cnlid, livepid, defn);
      if (!playUrl) return res.status(502).end('获取播放地址失败');
      const m3u8 = await fetchAndProxyM3u8(playUrl);
      if (!m3u8) return res.status(502).end('获取M3U8失败');
      res.setHeader('Content-Type', 'application/vnd.apple.mpegurl');
      return res.end(m3u8);
    }

    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    return res.end(`央视频 IPTV\n\nM3U订阅: ${baseUrl}/m3u\n单频道:  ${baseUrl}/live?id=cctv1`);
  } catch (e) {
    console.error(e);
